...
How **corpmap** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `corpmap` lives in the **Sanctions - ownership - finance** cluster.
...
| from | relation | to |
|---|---|---|
| `corpmap` | JSON findings compose with | [`cryptotrace`](https://github.com/cognis-digital/cryptotrace), [`maritimeint`](https://github.com/cognis-digital/maritimeint) |
| `corpmap` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `corpmap` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `corpmap` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the sanctions - ownership - finance cluster.** `corpmap` output feeds the next tool:
```bash
corpmap ... --format json > out.json          # this tool's findings (see `corpmap --help`)
cryptotrace ... < out.json                      # the cluster sibling consumes them
```
...
corpmap ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
corpmap ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
corpmap ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `corpmap`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `corpmap` slots
