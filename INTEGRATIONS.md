...
**corpmap** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `corpmap` emits JSON findings, pipe them straight to a destination — `--dry-run`
...
works whatever `corpmap` outputs:

```python
from cognis_connect import normalize, misp
findings = [normalize(rec, source="corpmap") for rec in records]   # records = your JSON output
