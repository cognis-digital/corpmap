...
TOOL_NAME = "CORPMAP"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'CM-SANC-001', 'severity': 'critical', 'weight': 3.0, 'title': 'SANCTIONED_ENTITY_LINK', 'when': {'sanctioned_owner': True}, 'remediation': 'Halt onboarding; escalate to compliance.'}, {'id': 'CM-PEP-001', 'severity': 'high', 'weight': 2.5, 'title': 'PEP_BENEFICIAL_OWNER', 'when': {'pep': True}, 'remediation': 'Enhanced due diligence per FATF guidance.'}, {'id': 'CM-SHELL-001', 'severity': 'medium', 'weight': 2.0, 'title': 'SHELL_STRUCTURE', 'when': {'shell_score': '^([6-9]|10)'}, 'remediation': 'Verify operations; possible layering structure.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="corp-graph",
                    metadata=item,
                ))
