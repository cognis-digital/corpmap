"""Native cognis-connect emit for corpmap — forward findings to any platform.

Maps corpmap's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    corpmap ... --format json | corpmap-emit --to stix
    corpmap-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a corpmap record toward the Finding contract before normalize().
